...
def main():
    ...
    # Wrap each factor’s key inside a unique object so that every node gets its own id,
    # even if the underlying label (i.e. the tuple content) is identical.
    class NodeKey:
        def __init__(self, tup, idx):
            self.tup = tup      # original tuple (i, j, ref)
            self.id = f"node_{idx}"
        def __hash__(self):
            return hash(self.id)
        def __eq__(self, other):
            return isinstance(other, NodeKey) and self.id == other.id
        def __str__(self):
            return self.id
        # Allow tuple-like indexing for comparisons and extracting text.
        def __getitem__(self, index):
            return self.tup[index]

    # Create a mapping from original keys to new ones with unique ids.
    node_map = {}
    new_factors = {}
    counter = 0
    for key, children in factors.items():
        new_key = NodeKey(key, counter)
        counter += 1
        node_map[key] = new_key
        new_factors[new_key] = children

    # Replace the original factors dictionary with one keyed by the unique NodeKey objects.
    factors.clear()
    factors.update(new_factors)

    # In children lists, if a child’s third element is a factor key, replace it with its unique id.
    for parent, children in factors.items():
        if children is not None:
            new_children = []
            for child in children:
                if isinstance(child[2], tuple) and child[2] in node_map:
                    # Replace the reference with the unique id
                    new_children.append((child[0], child[1], node_map[child[2]]))
                else:
                    new_children.append(child)
            factors[parent] = new_children
