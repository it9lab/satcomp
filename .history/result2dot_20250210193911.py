    ...
    for parent, children in factors.items():
        if children is None:
            if not isinstance(parent[2], tuple):
                dot_str += f'  "{parent[0],parent[1]}" [label="{text[parent[0]:parent[1]]}"];\n'
                rank_str += f'"{parent[0],parent[1]}"; '
        else:
            for c in children:
                if isinstance(c[2], tuple):
                    dot_str += f'  "{parent[0],parent[1]}" -> "{c[2][0],c[2][1]}";\n'
                    dot_str += f'  "{c[2][0],c[2][1]}" -> "{num_csref}";\n'
                    dot_str += f'  "{num_csref}" [label="{text[c[0]:c[1]]}"];\n'
                    rank_str += f'  "{num_csref}"; '
                    num_csref += 1
                else:
                    dot_str += f'  "{parent[0],parent[1]}" -> "{c[0],c[1]}";\n'
    rank_str += "}\n"
    dot_str += rank_str
