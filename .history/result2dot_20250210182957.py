...
def cs2str(root, cs, nstr, estr, rstr):
    # print(f"root={root}")
    res = []
    (i, j, ref) = root
    ...
    if j - i == 1:
        ...
        if ref == None: # SLPの生成規則を表す内部ノードの場合
            ...
            if children[0][2]  == "RLrule" and len(children) == 1:
                res += cs2str(children[0], cs, nstr, estr, rstr)
            else:
                res += cs2str(children[0], cs, nstr, estr, rstr)
                res += cs2str(children[1], cs, nstr, estr, rstr)
            ...
            if isinstance(ref, tuple):
                n = (ref[0], ref[1], None)
                res += cs2str(n, cs, nstr, estr, rstr)[ref[2]:ref[2]+(j-i)]
            elif (ref, ref + j - i, None) in cs:
                n = (ref, ref + j - i, None)
                res += cs2str(n, cs, nstr, estr, rstr)
            else:
...
def main():
    ...
    cs2str(root, factors, node_str, edge_str, rank_str)
    print("node_str:", node_str)
    print("edge_str:", edge_str)
    print("rank_str:", rank_str)
