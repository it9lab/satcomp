...
# SLPの解析木の情報を保存
def recover_cs(text: bytes, pstartl, refs_by_slpreferred, refs_by_rlreferred, refs_by_csreferres):
    n = len(text)
    # 各区間に対応するノードの種類を分類
    ...
    csreferred = set((refs_by_csreferres[i, l1][0], refs_by_csreferres[i, l1][1], i, l1) for (i, l1) in refs_by_csreferrer.keys())
    ...
    leaves.extend([(j, j + l, refs_by_csreferrer[j, l]) for (j, l) in refs_by_csreferrer.keys()])
