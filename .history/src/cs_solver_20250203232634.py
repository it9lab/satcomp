...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    n = len(text)
    # 各区間に対応するノードの種類を分類
    slpreferred = set((refs_by_slpreferrer[i, l], l) for (j, l) in refs_by_slpreferrer.keys()) # 参照元の位置と長さを保持するタプルを生成
    
    rliterated = set((refs_by_rlreferrer[j, l], j, l) for (j, l) in refs_by_rlreferrer.keys())

    # 切り取り規則を用いているかどうか
    # csreferredに(j, l2, i, l1)のペアを入れていく
    csreferred = set((refs_by_csreferrer[i, l1][0], refs_by_csreferrer[i, l1][1], i, l1) for (i, l1) in refs_by_csreferred.keys())
