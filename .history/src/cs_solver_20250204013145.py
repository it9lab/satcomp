...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    ...
    rlinternal = [(occ, i + l , "RLrule") for (occ, j, l) in rliterated] # 連長圧縮全体を表す内部ノードの区間(RLSLP)
    csinternal = [(j,  + l2, "CollageSystem") for (j, l2, i, l1) in csreferred] # 切り取り規則を表す内部ノードの区間(CS)
    #csinternal = 
