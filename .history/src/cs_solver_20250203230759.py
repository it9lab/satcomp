...
def recover_cs(text: bytes, pstartl, refs_by_slpreferred, refs_by_rlreferred, refs_by_csreferred):
    ...
    rliterated = set((refs_by_rlreferred[j, l], j, l) for (j, l) in refs_by_rlreferrer.keys())
    ...
    leaves = [(j, j + l, refs_by_slpreferrer[j, l]) for (j, l) in refs_by_slpreferrer.keys()]# 葉ノードが表す区間
    leaves.extend([(j, j + l, refs_by_rlreferrer[j, l]) for (j, l) in refs_by_rlreferrer.keys()])
