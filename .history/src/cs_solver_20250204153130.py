...
def cs2str(root, cs):
    ...
    if j - i == 1:
        ...
        if ref == None: # SLPの生成規則を表す内部ノードの場合
            ...
            elif isinstance(children[0][2], tuple):
                res += cs2str(children[0], cs)
            else:
                res += cs2str(children[0], cs)
...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    ...
    leaves.extend([(i, i + l, (refs_by_csreferrer[i, l][0], refs_by_csreferrer[i, l][1], None)) for (i, l) in refs_by_csreferrer.keys()])
        ...
        for (j, l2) in refs_by_csreferrer[i, l1]:
            if lm.getid(lm.lits.csref, j, l2, i, l1) in sol:
                for k in range(j, j + l2):
                    assert lm.getid(lm.lits.pstart, k) in sol
