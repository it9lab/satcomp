...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    ...
    # 切り取り規則を用いているかどうか
    # csreferredに（参照文字列の開始位置，（参照ノードの開始位置，参照先の長さ））を格納
    csreferred = set((refs_by_csreferrer[i, l][0], refs_by_csreferrer[i, l][1]) for (i, l) in refs_by_csreferrer.keys())
