...
# SLPの解析木の情報を保存
def recover_cs(text: bytes, pstartl, refs_by_slpreferred, refs_by_rlreferred, refs_by_csreferred):
    n = len(text)
    # 各区間に対応するノードの種類を分類
    slpreferred = set((refs_by_slpreferred[j, l], l) for (j, l) in refs_by_slpreferred.keys()) # 参照元の位置と長さを保持するタプルを生成
    
    rliterated = set((refs_by_rlreferred[j, l], j, l) for (j, l) in refs_by_rlreferred.keys())

    # 切り取り規則を用いているかどうか
    # csreferredに(j, l2, i, l1)のペアを入れていく
    csreferred = set((refs_by_csreferred[i, l1][0], refs_by_csreferred[i, l1][1], i, l1) for (i, l1) in refs_by_csreferres.keys())

    # ノードが内部ノードかつ蓮長圧縮ルール全体のノードとみなされていた場合，連長圧縮ルール全体のノードとして扱う
    # 繰り返しを表す区間が左に出現していた時発生する
    # 切り取り規則においては，leavesにreferrerを入れるのみ
    leaves = [(j, j + l, refs_by_slpreferred[j, l]) for (j, l) in refs_by_slpreferred.keys()]# 葉ノードが表す区間
    leaves.extend([(j, j + l, refs_by_rlreferred[j, l]) for (j, l) in refs_by_rlreferred.keys()])
    leaves.extend([(j, j + l, refs_by_csreferred[j, l]) for (j, l) in refs_by_csreferrer.keys()])
    ...
    for (i, l) in slpreferred:
        for (j, rl) in refs_by_rlreferred.keys():
            if i == refs_by_rlreferred[j, rl] and i + l == j + rl:
