...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    ...
    leaves = [(j, j + l, refs_by_slpreferrer[j, l]) for (j, l) in refs_by_slpreferrer.keys()] 
    + [(j, j + l, refs_by_rlreferrer[j, l]) for (j, l) in refs_by_rlreferrer.keys()] + [(j, j + l, refs_by_csreferrer[j, l]) for (j, l) in refs_by_csreferrer.keys()]# 葉ノードが表す区間
