...
def smallest_CollageSystem_WCNF(text: bytes):
    ...
    for (j, l2) in refs_by_rliterated.keys():
        for l in refs_by_rliterated[j, l2]:
            ...
            wcnf.append(pysat_if(qid, lm.getid(lm.lits.pstart, j)))
            wcnf.append(pysat_if(qid, lm.getid(lm.lits.dref, j, l, j+l2)))
    # // end constraint (7) ###############################
        ...
        for l in range(1, n - i + 1):
            ...
            for d in range(1, n):
                if lm.getid(lm.lits.depth, i, l, d) in sol and d > depth[i, l]:
                    depth[i, l] = d
    drefs = {}
    for (j, l) in refs_by_slpreferrer.keys() or refs_by_rlreferrer.keys() or refs_by_csreferrer.keys():
        for i in refs_by_slpreferrer[j, l] or refs_by_rlreferrer[j, l] or refs_by_csreferrer[j, l]:
            if lm.getid(lm.lits.dref, j, l, i) in sol:
                drefs[j, l] = i
