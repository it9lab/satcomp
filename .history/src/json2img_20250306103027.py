...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python src/result2img.py [solver_type(slp/rlslp/cs)] [string]")
        sys.exit(1)
    ...
    # DOTファイルを書き込み
    with open('dot/'+solver_type+'_result.dot', 'w') as f:
        f.write(dot_str)

    # DOTファイルをPNGファイルに変換
    import os
    os.system('dot -Tpng dot/'+solver_type+'_result.dot -o img/'+solver_type+'_result.png')
