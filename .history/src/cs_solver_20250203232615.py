...
# SLPの解析木の情報を保存
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferred, refs_by_csreferred):
    n = len(text)
    # 各区間に対応するノードの種類を分類
    ...
    csreferred = set((refs_by_csreferred[i, l1][0], refs_by_csreferred[i, l1][1], i, l1) for (i, l1) in refs_by_csreferred.keys())
    ...
    leaves.extend([(j, j + l, refs_by_csreferred[j, l]) for (j, l) in refs_by_csreferred.keys()])
