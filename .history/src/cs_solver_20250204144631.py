...
def smallest_CollageSystem_WCNF(text: bytes):
    ...
    # // end constraint (11) ############################### 
    for (i,l) in refs_by_csreferred.keys():
        for (j,l2) in refs_by_csreferred[i,l]:
            id1 = lm.getid(lm.lits.csref, i, l, j, l2)
            for d in range(1,n):
                id2 = lm.getid(lm.lits.depth, i, l, d)
                id3 = lm.getid(lm.lits.depth, j, l2, d-1)
                wcnf.append([-id1, id2, -id3])
...
def cs2str(root, cs):
    ...
    if j - i == 1:
        ...
        # print(f"root = {root}")
        # print(f"children = {children}")
        if ref == "SLP": # SLPの生成規則を表す内部ノードの場合
            ...
            for j in range(int((children[1][1] - children[0][0]) / (children[0][1] - children[0][0]))):
                res += cs2str(children[0], cs)
        elif ref == "CollageSystem": # 切り取り規則を表す葉ノードの場合
            res += cs2str(ref, cs)
...
def recover_cs(text: bytes, pstartl, refs_by_slpreferrer, refs_by_rlreferrer, refs_by_csreferrer):
    ...
    # 切り取り規則を用いているかどうか
    # csreferredに(j, l2)のペアを入れていく
    csreferred = set((refs_by_csreferrer[i, l1][0], refs_by_csreferrer[i, l1][1]) for (i, l1) in refs_by_csreferrer.keys())

    # ノードが内部ノードかつ蓮長圧縮ルール全体のノードとみなされていた場合，連長圧縮ルール全体のノードとして扱う
    # 繰り返しを表す区間が左に出現していた時発生する
    # 切り取り規則においては，leavesにreferrerを入れるのみ
    leaves = [(i, i + l, refs_by_slpreferrer[i, l]) for (i, l) in refs_by_slpreferrer.keys()]# 葉ノードが表す区間
    leaves.extend([(i, i + l, refs_by_rlreferrer[i, l]) for (i, l) in refs_by_rlreferrer.keys()])
    leaves.extend([(i, i + l, refs_by_csreferrer[i, l]) for (i, l) in refs_by_csreferrer.keys()])
    ...
    internal = [(occ, occ + l, "SLP") for (occ, l) in slpreferred] # 内部ノードが表す区間(SLP)
    rlinternal = [(occ, i + l , "RLrule") for (occ, i, l) in rliterated] # 連長圧縮全体を表す内部ノードの区間(RLSLP)
    csinternal = [(occ, occ + l2, "CollageSystem") for (occ, l2) in csreferred] # 切り取り規則を表す内部ノードの区間(CS)
