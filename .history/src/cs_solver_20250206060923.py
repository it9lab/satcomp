...
def smallest_CollageSystem(text: bytes, exp: Optional[SLPExp] = None) -> SLPType:
    ...
    cssize = len(posl) - 2 + len(set(text)) + len(csrefs)
...
if __name__ == "__main__":
    ...
    if args.output == "":
        print(exp.tojson(ensure_ascii=False))  # type: ignore
    else:
        with open(args.output, "w") as f:
            exp2 = exp.to_json(ensure_ascii=False)
            json.dump(exp2, f, ensure_ascii=False)
