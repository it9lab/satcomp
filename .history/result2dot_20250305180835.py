...
def main():
    ...
    unique_ids = {}
    uid_counter = 0
    # Create a unique identifier for each node (both parents and children)
    for node in list(factors.keys()):
        if node not in unique_ids:
            unique_ids[node] = f"node_{uid_counter}"
            uid_counter += 1
        children = factors[node]
        if children is not None:
            for child in children:
                if child not in unique_ids:
                    unique_ids[child] = f"node_{uid_counter}"
                    uid_counter += 1
                # If the node's reference is a tuple, register it too
                if isinstance(child[2], tuple) and child[2] not in unique_ids:
                    unique_ids[child[2]] = f"node_{uid_counter}"
                    uid_counter += 1
