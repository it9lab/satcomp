        ...
        if children is None:
            ...
            for c in sorted(children, key=lambda x: x[0]):
                ...
                if isinstance(c[2], tuple):
                    ...
                    dot_str += f'  "{csref_id}" [label="{text[c[0]:c[1]]}"];\n'
                    assert text[c[0]:c[1]] == text[c[2][2]+c[2][0]:c[2][2]+c[2][0]+(c[1]-c[0])]
                    rank_nodes.append((csref_id, c[0]))
                    num_csref += 1
                else:
                    if c[1] - c[0] == 1 or c[2] == 'RLrule' or c[2] == None:
                        dot_str += f'  "{parent_id}" -> "{child_id}";\n'
                    elif parent[2] == 'RLrule' and parent[0] == c[2]:
                        dot_str += f'  "{parent_id}" -> "{child_id}" [label="RestRL"];\n'
                    else:
                        dot_str += f'  "{parent_id}" -> "{child_id}" [label="SLP{c[2],c[2]+c[1]-c[0]}"];\n'
